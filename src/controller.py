...
class Controller(qtc.QObject):
	def __init__(self, view, image_extractor, prediction_analyzer):
		super().__init__()
		self.view = view 
	...
	def set_up_threads(self):
		...
		#self.image_extractor.moveToThread(self.extractor_thread)
		#self.extractor_thread.started.connect(self.image_extractor.run)
	...
	def connect_view_to_extractor(self):
		...
		self.view.extraction_frame.info.connect(lambda widget_info: self.image_extractor.receive_info(widget_info))
		self.view.extraction_frame.start.connect(lambda: self.image_extractor.reset())
		self.view.extraction_frame.start.connect(lambda: self.extractor_thread.start())
		self.image_extractor.progress_value.connect(lambda value: self.view.update_progress_bar_value(value))
		self.image_extractor.progress_text.connect(lambda text: self.view.update_progress_text(text))
		self.image_extractor.finished.connect(lambda: self.extractor_thread.quit())
